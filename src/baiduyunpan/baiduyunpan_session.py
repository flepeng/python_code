...
"""
    ...
    @Author: Feng Lepeng
    @File  : baidupan.py
    @Desc  : 基于 session 的百度网盘脚本
            初次使用时，请先从浏览器的开发者工具中获取百度网盘的 Cookie，并在 init 方法中进行配置，并调用 verifyCookie 方法测试 Cookie 的有效性
            已实现的方法：
                1.获取登录Cookie有效性；
                2.获取网盘中指定目录的文件列表；
                3.获取加密分享链接的提取码；
...
class BaiDuPan(object):
    ...
    def __init__(self, cookies_bduss=None, cookies_stoken=None):
        # 创建 session 并设置初始登录 Cookie
        self.session = requests.session()
        self.session.cookies['BDUSS'] = cookies_bduss
        self.session.cookies['STOKEN'] = cookies_stoken
        self.headers = {
            'Host': 'pan.baidu.com',
    ...
    def verify_cookie(self):
        """
        验证Cookie是否已登录，
        :return: 返回值errno代表的意思：0 有效的Cookie；1 init 方法中未配置登录 Cookie；2 无效的Cookie
        """
    ...
    def get_file_list(self, dir='/', order='time', desc=0, page=1, num=100):
        """
        获取指定目录的文件列表，直接返回原始的 json
        :param dir: 需要获取的目录
        :param order: 可能值：name，time，size
        :param desc: 0表示正序，1表示倒序
        :param page:  第几页
        :param num: 每页文件数量
        """
        t = random.random()
        start_log_time = str(int(time.time()) * 1000)
        """
        构造获取文件列表的URL：https://pan.baidu.com/api/list?
            bdstoken=  从首页中可以获取到
            &dir=/  需要获取的目录
        ...
        url = 'https://pan.baidu.com/api/list?bdstoken=%s&dir=%s&order=%s&desc=%s&page=%s&num=%s&t=%s&startLogTime=%s\
              ...
              % (self.bdstoken, dir, order, desc, page, num, t, start_log_time)
    ...
    def get_share_pwd(surl):
        ...
        获取分享链接的提取码
        :param surl:
        :return: 返回值errno代表的意思：0 提取码获取成功；1 提取码获取失败；
        """
        ypsuperkey_headers = {
    ...
    def vcode_OCR(self):
        """
        识别 验证加密分享时的验证码
        :return: 返回值errno代表的意思：0 识别成功；1 识别失败；其他值 获取验证码失败；
        """
    ...
    def verify_share(self, surl, bdstoken, pwd, referer):
        ...
        :param pwd:
        :param referer:
        :return: 返回值errno代表的意思：0 加密分享验证通过；1 验证码获取失败；2 提取码不正确；3 加密分享验证失败；4 重试几次后，验证码依旧不正确；
        """
    ...
    def save_share(self, url, pwd=None, path='/'):
        ...
        if ('init' in share_res.url):
            ...
            verify_result = self.verify_share(surl, bdstoken, pwd, referer)
    ...
    def create_share_link(self, fid_list, period=0, pwd=False):
        """
        创建分享链接
        :param fid_list: fid_list 为列表，例如：[1110768251780445]
        :param period:
        :param pwd:
...
if __name__ == '__main__':
    obj = BaiDuPan()
    print(obj.verify_cookie())
    obj.get_file_list('/书籍')
    print(obj.create_share_link('[165681573819616]'))
